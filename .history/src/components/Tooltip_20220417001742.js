...
const Tooltip = ({
  ...
  const drawContent = React.useCallback(
    (x) => {
      ...
      tooltipContent
        .select(".contentTitle")
        .text(d3.timeFormat("DATE_HUGE TIME_24_WITH_SECONDS")(xScale.invert(x)));
    },
    [xScale, margin, width]
  );
  ...
  const drawBackground = React.useCallback(() => {
    ...
    const contentBackground = d3
      ...
      .attr('stroke', '#7CB5EC')
      .attr('strokeWidth', '3')
      .attr("style", 'filter: drop-shadow( 0px 2px 3px #666)')
  ...
  const followPoints = React.useCallback(() => {
    ...
    d3.select(ref.current)
      ...
      .attr("transform", (cur, i) => {
        ...
        return `filter: drop-shadow( 0px 2px 3px ${data[i].color}80)`;
      });
